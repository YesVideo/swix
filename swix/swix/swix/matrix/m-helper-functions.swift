...
// NORMs
public func norm(x:matrix, ord:String="assumed to be 'fro' for Frobenius")->Double{
    if ord == "fro" {return norm(x.flat, ord:2)}
    assert(false, "Norm type assumed to be \"fro\" for Forbenius norm!")
    return -1
}
public func norm(x:matrix, ord:Double=2)->Double{
...
public func det(x:matrix)->Double{
...
public func kron(A:matrix, B:matrix)->matrix{
    // an O(n^4) operation!
...
public func tril(x: matrix) -> ndarray{
    let (m, n) = x.shape
...
public func triu(x: matrix)->ndarray{
    let (m, n) = x.shape
    let (mm, nn) = meshgrid(arange(m), y: arange(n))
    var i = mm - nn
    let j = (i > 0-S2_THRESHOLD)
...
// PRINTING
public func println(x: matrix, prefix:String="matrix([", postfix:String="])", newline:String="\n", format:String="%.3f", printWholeMatrix:Bool=false){
...
public func max(x: matrix, axis:Int = -1)->Double{
    return x.max()
}
public func min(x: matrix, axis:Int = -1)->Double{
    return x.min()
}
public func print(x: matrix, prefix:String="matrix([", postfix:String="])", newline:String="\n", format:String="%.3f", printWholeMatrix:Bool=false){
