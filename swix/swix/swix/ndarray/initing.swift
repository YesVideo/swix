...
public func zeros(N: Int) -> ndarray{
    // N zeros
    return ndarray(n: N)
}
public func zeros_like(x: ndarray) -> ndarray{
    // make an array like the other array
   return zeros(x.n)
}
public func ones_like(x: ndarray) -> ndarray{
    // make an array like the other array
    return zeros_like(x) + 1
}
public func ones(N: Int) -> ndarray{
    // N ones
    return ndarray(n: N)+1
}
public func arange(max: Double, x exclusive:Bool = true) -> ndarray{
    // 0..<max
    return arange(0, max: max, x:exclusive)
}
public func arange(max: Int, x exclusive:Bool = true) -> ndarray{
    // 0..<max
    return arange(0, max: max.double, x:exclusive)
}
public func range(min:Double, max:Double, step:Double) -> ndarray{
    // min, min+step, min+2*step..., max-step, max
    return linspace(min, max: max, num:1+((max-min)/step).int)
}
public func arange(min: Double, max: Double, x exclusive: Bool = true) -> ndarray{
    // min...max
    var pad = 0
    if !exclusive {pad = 1}
...
public func linspace(min: Double, max: Double, num: Int=50) -> ndarray{
    // 0...1
    let x = zeros(num+0)
    var min  = CDouble(min)
...
public func array(numbers: Double...) -> ndarray{
...
public func asarray(seq: Range<Int>) -> ndarray {
...
public func copy(x: ndarray) -> ndarray{
    // copy the value
    return x.copy()
}

public func seed(n:Int){
    SWIX_SEED = __CLPK_integer(n)
}

public func rand(N: Int, distro:String="uniform") -> ndarray{
    let x = zeros(N)
...
public func randn(N: Int, mean: Double=0, sigma: Double=1) -> ndarray{
    return (rand(N, distro:"normal") * sigma) + mean;
}
public func randperm(N:Int)->ndarray{
    let x = arange(N)
    let y = shuffle(x)
    return y
}
